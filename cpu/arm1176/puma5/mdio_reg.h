...
#define pMDIO_VER(base)                 ((volatile UINT32 *)(base+0x00))
#define pMDIO_CONTROL(base)             ((volatile UINT32 *)(base+0x04))
#define pMDIO_ALIVE(base)               ((volatile UINT32 *)(base+0x08))
#define pMDIO_LINK(base)                ((volatile UINT32 *)(base+0x0C))
#define pMDIO_LINKINTRAW(base)          ((volatile UINT32 *)(base+0x10))
#define pMDIO_LINKINTMASKED(base)       ((volatile UINT32 *)(base+0x14))
#define pMDIO_USERINTRAW(base)          ((volatile UINT32 *)(base+0x20))
#define pMDIO_USERINTMASKED(base)       ((volatile UINT32 *)(base+0x24))
#define pMDIO_USERINTMASKED_SET(base)   ((volatile UINT32 *)(base+0x28))
#define pMDIO_USERINTMASKED_CLR(base)   ((volatile UINT32 *)(base+0x2C))
#define pMDIO_USERACCESS(base, channel) ((volatile UINT32 *)(base+(0x80+(channel*8))))
#define pMDIO_USERPHYSEL(base, channel) ((volatile UINT32 *)(base+(0x84+(channel*8))))
...
static unsigned short MdioRd(unsigned int MdioBase, int ch, 
                                int PhyAddr, int RegNum)
{
    /* Wait for SM to finish any prior transaction */
    while(MDIO_USERACCESS(MdioBase,ch)&(1u<<31));      
     
    MDIO_USERACCESS(MdioBase,ch) = (1u<<31)|(RegNum<<21)|(PhyAddr<<16);
  
    /* Wait for current read to complete */
     while(MDIO_USERACCESS(MdioBase,ch)&(1u<<31));      
     
    return MDIO_USERACCESS(MdioBase,ch)&0xFFFF;
}

static void MdioWr(unsigned int MdioBase, int ch, 
                    int PhyAddr, int RegNum, unsigned short data)
{
    /* Wait for SM to finish any prior transaction */
    while(MDIO_USERACCESS(MdioBase,ch)&(1u<<31));

    MDIO_USERACCESS(MdioBase,ch) = (1u<<31) | 
                                    (1<<30) | 
                                    ((RegNum&0x1F)<<21) | 
                                    ((PhyAddr&0x1F)<<16) | 
                                    (data&0xFFFF);
...
static void MdioSetBits(unsigned int MdioBase, int ch, 
                        int PhyAddr, int RegNum, unsigned short BitMask)
{
    MdioWr (MdioBase, ch, PhyAddr, RegNum, 
            MdioRd(MdioBase, ch, PhyAddr,RegNum) |
            BitMask); 
}

static void MdioClearBits(unsigned int MdioBase, int ch, 
                        int PhyAddr, int RegNum, unsigned short BitMask)
{
    MdioWr (MdioBase, ch, PhyAddr, RegNum, 
            MdioRd(MdioBase, ch, PhyAddr,RegNum) &
...
#define NWAY_GIG_ADVERTIZE_REG  9  
